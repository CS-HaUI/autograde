...
image = cv2.imread("dataset\\IMAGE\\002.jpg")
gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)


gray = cv2.bilateralFilter(gray, 20, 70, 70)

img = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 151, 4)

edges_image = cv2.morphologyEx(img, cv2.MORPH_CLOSE, np.ones((5, 5)), iterations= 5)
